...
模型、推理深度和编排模式由工作区路由策略与当前任务契约决定。此技能声明 `decomposition`、`dependency_analysis`、`writing` 和 `verification` 能力需求；跨项目或大规格任务应由契约提升风险与复核强度，不固定模型名。
...
4. **拆 Story** — 每 Epic 拆 2-5 个可独立验收的垂直切片；文件数是范围信号，不是硬编码上限
...
- Context 段提供最小充分引用；执行者仍必须读取适用的 `AGENTS.md` 和任务契约
